...
let fromList lst =
    ...
    let () = List.iter
                (fun (x, y) ->
                    if x >= 0 && y >= 0 then
                        Hashtbl.replace canvas.data {x; y} setPixel
                    else
                        failwith "Negative coordinate passed to fromList"
                ) lst in
    canvas

(** Dimensions of the canvas. *)
let dimensions canvas =
    let (max_x, max_y) = Hashtbl.fold
        (fun c _ (x, y) -> (max x c.x, max y c.y))
        canvas.data (0, 0)
    in
    (max_x + 1, max_y + 1)
...
let get canvas pixelCoord =
    try
        Hashtbl.find canvas.data pixelCoord
    with Not_found -> unsetPixel

let updateWithValue canvas coord value =
    if coord.x < 0 || coord.y < 0 then
        failwith "Negative coordinate passed to updateWithValue"
    else
        if value = setPixel then
            Hashtbl.replace canvas.data coord setPixel
        else
            Hashtbl.remove canvas.data coord

let set canvas coord = updateWithValue canvas coord setPixel

let unset canvas coord = updateWithValue canvas coord unsetPixel

let toggle canvas coord =
    updateWithValue canvas coord @@
                    invertValue @@ get canvas coord
...
let toDebugString canvas =
    ...
    for row = 0 to size_y - 1 do
        for col = 0 to size_x - 1 do
            Bytes.set bitmap (row*(size_x + 1) + col)
...
let toBrailleBitmap canvas =
    ...
    for row = 0 to size_y do
        ...
        for col = 0 to size_x do
            ...
            if get canvas { x = col; y = row } then
                Bytes.set bitmap braileIndex
...
let printBrailleBitmap (_, b_size_y, bitmap) =
    let value col row =
        Char.code (Bytes.get bitmap (b_size_y * row + col))
    in
...
let frame canvas =
    ...
    for row = 0 to b_size_y - 1 do
        ...
        for col = 0 to b_size_x - 1 do
            ...
            let v1 =  0xA0 + (c land pixmap.(3).(0))/pixmap.(3).(0)
